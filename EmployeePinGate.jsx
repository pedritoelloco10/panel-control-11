...
export default function EmployeePinGate({ employees, onIdentify }) {
  const [pin, setPin] = useState("");
  const [err, setErr] = useState(false);

  function submit(value) {
    const match = employees.find((e) => e.pin === value && e.activo);
  ...
  return (
    <div className="max-w-xs mx-auto mt-16 text-center pt-5">
      ...
      <p className="text-slate-500 text-xs mb-5">Ingresá tu PIN para empezar el turno</p>
        ...
        {["1", "2", "3", "4", "5", "6", "7", "8", "9", "", "0", "⌫"].map((d, i) =>
          d === "" ? <div key={i} /> : (
            <button
              ...
              className="bg-white/5 ring-1 ring-white/10 rounded-xl py-3.5 text-lg font-bold text-slate-200 active:bg-white/10"
            >
              {d}
            </button>
          )
